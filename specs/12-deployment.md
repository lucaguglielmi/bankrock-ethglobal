...
| **Run 22**, on `8108580`, 14:12–14:18 UTC | Nowhere — and this time it counts | After the token was extended (Workers Scripts, Workers Routes, DNS), `wrangler deploy` published the Worker `web`. The apex went from `1789198731586` to `1789222449317` at 14:18:34 UTC; `/`, `/r/1`, `/rock/1`, `/sw.js` and the manifest all answer 200 with `x-opennext: 1`. **The first build from this repository to reach a user.** `www` still 308s to the literal `:path*` — that is the dashboard rule (DEMO-STATE W-1), not the deploy |
