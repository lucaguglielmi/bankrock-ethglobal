...
| 2 | ~~Delete the old `www` redirect rule~~ **Not an operator item after all.** The zone API showed no redirect ruleset; the literal `:path*` came from the app's own redirect for the empty path (OpenNext quirk). Fixed in `web/next.config.ts` on 2026-09-12 | — | `bash scripts/check-live.sh` www line green |
| 3 | ~~Create the Privy app~~ **App created 2026-09-12** (`cmtytvqip029c0djknzi8qz4t`, committed in `wrangler.jsonc`). Still to confirm in the dashboard: allowed origin `https://bank-rock.com`, email + passkey login, Sepolia enabled and first, and **"Automatically create embedded wallets on login" ON with EVM wallets ticked** (the app also requests it in `providers.tsx`, so a fresh email sign-in gets a wallet either way) | dashboard.privy.io | Beat 4 |
...
| 5 | ~~Set the Worker secrets~~ **done 2026-09-12** (nine secrets via `wrangler secret bulk`; plain variables are in git, D-034) | Workers & Pages → `web` → Settings → Variables and Secrets → *Secret*: `SEPOLIA_RPC_URL`, `PIMLICO_API_KEY`, `ATTESTATION_SIGNER_PRIVATE_KEY`, `NXP_MASTER_KEY`, `RELAYER_PRIVATE_KEY`, `FAUCET_PRIVATE_KEY`, `ADMIN_PASSWORD`, `ADMIN_JWT_SECRET`, `ADMIN_API_KEY`, `CRON_SECRET`. `RESEND_API_KEY` and `ALERT_EMAIL_ADDRESS` only if email is wanted (cuttable) | Everything server-side |
...
| 9 | ~~Run the live rehearsal~~ **done 2026-09-12 21:24 UTC** — `contracts/deployments/rehearsal-2026-09-12.md`, all five steps green, sponsored by Pimlico, relayed claim in the D-032 order | Its report lands in `contracts/deployments/rehearsal-<date>.md`; DEMO-STATE §5 P-2…P-5 are deleted on the first green run | Confidence that the chain path works before a phone touches it |
...
| 9 | ~~Pimlico sponsorship policy exists for chain 11155111~~ **done** — the rehearsal's every UserOperation was sponsored | — |
