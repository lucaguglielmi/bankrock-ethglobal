...
| P-12 | **The hosted MCP endpoint on a phone** | 11 §5, 12 §2 | `POST https://bank-rock.com/api/mcp` is exercised end to end by `web/src/app/api/mcp/mcp-route.test.ts` through the real SDK transport, and the tools it serves are the same reads the rock page makes. Proven when a phone client — ChatGPT with the connector created under Developer mode, or the Claude app with it added under Connectors — lists the tools and answers `get_rock_status` for rock 3 from the live site. Until then the per-client steps on bank-rock.com/mcp describe the vendors' documented flow, not one this project has watched succeed. |
