...
| --- | --- | --- | --- |
| S-1 | ~~Cross-chain deposit (the bridge modal)~~ **Retired.** The simulated modal is deleted (spec 20 D-035). Money from another chain now arrives for real through a Privy universal deposit address on the savings card — into the embedded wallet on Base, not into the Sepolia Rock Account. See P-12. | 20 Part 11 | — |
...
| P-12 | **Universal deposit addresses** | 20 Part 11, D-035 | `useDepositAddress` is wired with the vault's chain and asset as the destination; no deposit has been made. Proven when the button opens Privy's source list (not `DEPOSIT_ADDRESSES_NOT_ENABLED`), a 5 USDC deposit from a second chain lands as USDC on Base, and *Ready to add* moves without a refresh. |
| P-13 | **The rock-trading agent** | 20 Part 12, D-036 | The plan builder is tested against an encoded strategy and `--dry-run` against a stubbed API; no CLI session and no Sepolia transaction has been made. Proven when `privy-agent-wallet login` completes, the real run lands `approve` then `swapExactIn` with `status: success`, and the rock's virtual balances differ before and after. |
