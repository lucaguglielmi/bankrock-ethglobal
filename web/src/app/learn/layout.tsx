import * as React from "react";

/**
 * The GitHub mark, inline. lucide-react 1.x dropped its brand icons (`Github` no longer exists),
 * which broke the typecheck and the production build on main; the path below is the public
 * GitHub mark, sized by the same `size-4` class the icon used.
 */
function GithubMark({ className }: { className?: string }) {
  return (
    <svg
      viewBox="0 0 24 24"
      fill="currentColor"
      aria-hidden="true"
      focusable="false"
      className={className}
    >
      <path d="M12 .5C5.65.5.5 5.65.5 12c0 5.08 3.29 9.39 7.86 10.91.58.1.79-.25.79-.56v-2.17c-3.2.7-3.87-1.36-3.87-1.36-.52-1.33-1.28-1.68-1.28-1.68-1.04-.71.08-.7.08-.7 1.15.08 1.76 1.19 1.76 1.19 1.03 1.76 2.69 1.25 3.35.96.1-.75.4-1.25.73-1.54-2.55-.29-5.24-1.28-5.24-5.69 0-1.26.45-2.29 1.19-3.09-.12-.29-.52-1.46.11-3.05 0 0 .97-.31 3.17 1.18a11 11 0 0 1 5.78 0c2.2-1.49 3.17-1.18 3.17-1.18.63 1.59.23 2.76.11 3.05.74.8 1.19 1.83 1.19 3.09 0 4.42-2.7 5.39-5.26 5.68.41.36.78 1.06.78 2.14v3.17c0 .31.21.67.8.56A11.51 11.51 0 0 0 23.5 12C23.5 5.65 18.35.5 12 .5z" />
    </svg>
  );
}

export default function LearnLayout({ children }: { children: React.ReactNode }) {
  return (
    <>
      {children}
      <footer className="w-full bg-white pb-[calc(var(--dock-h,80px)+3rem)] pt-24 text-center flex justify-center">
        <a 
          href="https://github.com/lucaguglielmi/bankrock-ethglobal" 
          target="_blank" 
          rel="noopener noreferrer"
          className="flex items-center gap-2 rounded-full border border-border bg-white px-5 py-2.5 text-sm font-semibold text-ink-3 hover:bg-neutral-50 hover:text-ink transition-colors shadow-sm"
        >
          <GithubMark className="size-4" />
          <span>View project on GitHub</span>
        </a>
      </footer>
    </>
  );
}
