...
function SmallRock({ position, scale, speed, rotationIntensity, color = "#f8f8f8" }: any) {
  const meshRef = useRef<THREE.Mesh>(null);
  ...
  useFrame((state, delta) => {
    if (meshRef.current) {
      ...
      meshRef.current.rotation.x = Math.sin(state.clock.elapsedTime * 0.5 + randomOffset.current) * 0.2;
      meshRef.current.position.y += Math.sin(state.clock.elapsedTime * 0.8 + randomOffset.current) * 0.005;
    }
  });
  ...
  return (
    <div className="absolute inset-0 pointer-events-none z-0">
      <Canvas shadows camera={{ position: [0, 0, 6], fov: 45 }}>
        ...
        <SmallRock position={[-2.5, 1.2, -1]} scale={0.3} speed={2.5} rotationIntensity={1.5} />
        <SmallRock position={[2.2, -0.8, -0.5]} scale={0.2} speed={3} rotationIntensity={2} />
        <SmallRock position={[-1.5, -1.5, -2]} scale={0.4} speed={2} rotationIntensity={1} color="#f0f0f0" />
        <SmallRock position={[2.8, 1.5, -2]} scale={0.25} speed={2.8} rotationIntensity={1.8} />
