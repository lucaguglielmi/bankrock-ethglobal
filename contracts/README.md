...
Listed for completeness. `att` is the attestation struct
(`rockId`, `uidHash`, `counter`, `deadline`, `subject`, `smartAccount`) and `sig` is the
verifier's 65-byte signature over it. Both come from the app after a real tap; Etherscan cannot
make them. If you have a valid pair — for instance you captured one from your own tap and want to
relay it yourself — both calls work from any sender: the rock goes to `att.subject`, never to you.

**Expect:** `RockAwakened(rockId, rockOwner, uidHash, smartAccount, counter)` or
`HandoverClaimed(rockId, previousOwner, newOwner, counter)`.
...
| `to` | `0x0000…0000` | Who receives the output. All zeros means you. |
